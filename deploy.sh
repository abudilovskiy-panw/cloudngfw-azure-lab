...
#az resource create -g $rg --resource-type "PaloAltoNetworks.Cloudngfw/localRulestacks" -n "cloudngfw-lrs-"$region1 --properties @local_rulestack.json
#az resource create -g $rg --resource-type "PaloAltoNetworks.Cloudngfw/localRulestacks" -n "cloudngfw-lrs-"$region2 --properties @local_rulestack.json
...
az network vhub create -g $rg --name $hub1name --address-prefix 10.251.0.0/16 --vwan $vwanname --location $region1 --sku Standard --tags "hubSaaSPreview=true" --no-wait
az network vhub create -g $rg --name $hub2name --address-prefix 10.252.0.0/16 --vwan $vwanname --location $region2 --sku Standard --tags "hubSaaSPreview=true" --no-wait

echo Creating branches VNETs...
# create location1 branch virtual network
az network vnet create --address-prefixes 192.168.0.0/22 -n branch1 -g $rg -l $region1 --subnet-name main --subnet-prefixes 192.168.0.0/24 --output none

# create location2 branch virtual network
az network vnet create --address-prefixes 192.168.128.0/22 -n branch2 -g $rg -l $region2 --subnet-name main --subnet-prefixes 192.168.128.0/24 --output none
...
az vm create -n spoke4VM  -g $rg --image ubuntults --public-ip-sku Standard --size $vmsize -l $region2 --subnet main --vnet-name spoke4 --admin-username $username --admin-password $password --nsg "" --no-wait --only-show-errors
...
#az resource create -g $rg --resource-type PaloAltoNetworks.Cloudngfw/firewalls -n "cloudngfw-"$region1 --properties @cloudngfw.json
